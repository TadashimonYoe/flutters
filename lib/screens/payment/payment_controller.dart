...
import 'payment_gateways/tadashi_service.dart';
import 'dart:io';
...
class PaymentController extends GetxController {
  ...
  TadashiService tadashiService = TadashiService();

  TextEditingController tadashiName = TextEditingController();
  TextEditingController tadashiDesc = TextEditingController();
  Rx<File?> tadashiProofFile = Rx<File?>(null);
  RxBool isTadashiValid = false.obs;

  Future<void> pickTadashiProof() async {
    final XFile? picked = await ImagePicker().pickImage(
        source: ImageSource.gallery, maxWidth: 1800, maxHeight: 1800);
    if (picked != null) {
      tadashiProofFile(File(picked.path));
    }
  }
  ...
  Future<void> getAppConfigurations() async {
    ...
    await AuthServiceApis()
        .getAppConfigurations(forceSync: true)
        .then((value) async {
  ...
  Future<RxBool> getPayment({bool showLoader = true}) async {
    ...
    if (appConfigs.value.razorPay.razorpayPublickey.isNotEmpty) {
      originalPaymentList.add(
        PaymentSetting(
          ...
          liveValue: LiveValue(
              razorKey: appConfigs.value.razorPay.razorpayPublickey,
              razorSecret: appConfigs.value.razorPay.razorpaySecretkey),
        ),
      );
    }
    if (appConfigs.value.payStackPay.paystackPublickey.isNotEmpty) {
      originalPaymentList.add(
        PaymentSetting(
          ...
          liveValue: LiveValue(
              paystackPublicKey: appConfigs.value.payStackPay.paystackPublickey,
              paystackSecrateKey:
    ...
    if (appConfigs.value.paypalPay.paypalClientid.isNotEmpty) {
      originalPaymentList.add(
        PaymentSetting(
          ...
          liveValue: LiveValue(
              payPalClientId: appConfigs.value.paypalPay.paypalClientid,
              payPalSecretKey: appConfigs.value.paypalPay.paypalSecretkey),
        ),
      );
    }
    ...
    try {
      final PaymentSettingResponse res =
          await CoreServiceApis().getPaymentSettings();
      if (res.data.isNotEmpty) {
        // Merge without duplicating by type+title
        final existingKeys =
            originalPaymentList.map((e) => '${e.type}|${e.title}').toSet();
        for (final p in res.data) {
          final key = '${p.type}|${p.title}';
          if (!existingKeys.contains(key)) {
            originalPaymentList.add(p);
            existingKeys.add(key);
          }
        }
      }
    } catch (e) {
      log('Payment settings fetch error: ${e.toString()}');
    }
    isPaymentLoading(false);
    ...
    showInDialog(
      ...
      builder: (context) {
        return PlanConfirmationDialog(
          titleText:
              "${isRent.value ? locale.value.doYouConfirmThis(videoPlayerModel.value.name) : locale.value.doYouConfirmThisPlan}${selectPlan.value.name} ?",
          onConfirm: () {
            Get.back();
            if (paymentOption.value == PaymentMethods.PAYMENT_METHOD_STRIPE) {
              ...
              payWithPaypal(context, onSuccess);
                ...
                PaymentMethods.PAYMENT_METHOD_IN_APP_PURCHASE) {
              payWithPaypal(context, onSuccess);
            } else if (paymentOption.value ==
                PaymentMethods.PAYMENT_METHOD_TADASHI) {
              payWithTadashi(context, onSuccess);
            }
          },
        );
      },
  ...
  Future<void> payWithStripe(
    ...
    await StripeServices.stripePaymentMethod(
      ...
      amount:
          isRent.value ? rentPrice.value.validate() : price.value.validate(),
      onComplete: (res) {
        if (isRent.value) {
          saveRentDetails(
  ...
  Future<void> payWithRazorPay(
    ...
    razorPayService.init(
      razorKey: appConfigs
          .value.razorPay.razorpaySecretkey, //"rzp_test_CLw7tH3O3P5eQM"
      totalAmount:
          isRent.value ? rentPrice.value.validate() : price.value.validate(),
      onComplete: (res) {
        if (isRent.value) {
  ...
  Future<void> payWithTadashi(
      BuildContext context, VoidCallback onSuccess) async {
    if (isLoading.value) return;
    isLoading(true);
    toast('Procesando pago...', print: true);
    final Map<String, dynamic> req = {
      'user_id': loginUserData.value.id.toString(),
      'name': tadashiName.text.trim(),
      'description': tadashiDesc.text.trim(),
      'payment_type': PaymentMethods.PAYMENT_METHOD_TADASHI,
      'amount': isRent.value
          ? rentPrice.value.validate().toString()
          : price.value.validate().toString(),
      'plan_id': isRent.value ? '' : selectPlan.value.planId.toString(),
      'purchase_type':
          isRent.value ? videoPlayerModel.value.purchaseType : 'subscription',
    };
    try {
      if (tadashiName.text.trim().isEmpty || tadashiProofFile.value == null) {
        throw 'Completa nombre y adjunta comprobante';
      }
      String transactionId = '';
      try {
        final resp = await CoreServiceApis().submitTadashiPayment(
            request: req,
            files: tadashiProofFile.value != null
                ? [tadashiProofFile.value!]
                : null);
        transactionId =
            resp['data'] is Map && resp['data']['transaction_id'] != null
                ? resp['data']['transaction_id'].toString()
                : DateTime.now().millisecondsSinceEpoch.toString();
        successSnackBar('Comprobante enviado, pago pendiente de aprobación');
        if (isRent.value) {
          saveRentDetails(
            transactionId: transactionId,
            paymentType: PaymentMethods.PAYMENT_METHOD_TADASHI,
            onSuccess: onSuccess,
          );
        } else {
          Get.offAll(() =>
        ...
        errorSnackBar(
            error: 'Servidor sin endpoint Tadashi, se registró pago pendiente');
        await tadashiService.checkout(
          totalAmount: isRent.value ? rentPrice.value : price.value,
          onComplete: (map) {
...
//saveSubscriptionDetails
  ...
  Future<void> saveSubscriptionDetails(
    ...
    final Map<String, dynamic> request = {
      ...
      "payment_status": paymentType == PaymentMethods.PAYMENT_METHOD_TADASHI
    ...
    await CoreServiceApis()
        .saveSubscriptionDetails(
      request: request,
      proofFile: paymentType == PaymentMethods.PAYMENT_METHOD_TADASHI ? tadashiProofFile.value : null,
    )
        .then((value) async {
      if (paymentType == PaymentMethods.PAYMENT_METHOD_TADASHI) {
        successSnackBar('Tu pago está pendiente de aprobación');
      } else {
  ...
  Future<void> saveRentDetails(
    ...
    await CoreServiceApis()
        .saveRentDetails(request: request)
        .then((value) async {
      Get.back();

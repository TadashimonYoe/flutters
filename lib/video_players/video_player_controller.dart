...
class VideoPlayersController extends GetxController {
  ...
  late Player mkPlayer;
  VideoController? mkVideoController;
  ...
  Future<void> initializePlayer({String videoURL = '', String videoType = ''}) async {
    ...
    } else if (videoModel.value.type == VideoType.liveTv && videoUploadType.value.toLowerCase() != PlayerTypes.embedded.toLowerCase()) {
      isBuffering(true);
      mkPlayer = Player();
      mkVideoController = VideoController(mkPlayer);
      final headers = {
        'User-Agent': getUserAgent(),
      };
      try {
        await mkPlayer.open(Media(videoUrlInput.value, httpHeaders: headers));
      } catch (e) {
        errorMessage(e.toString());
      }
      mkPlayer.stream.buffering.listen((buffering) {
        isBuffering(buffering);
      });
      mkPlayer.stream.playing.listen((playing) {
...
''';
  ...
  Future<void> pause() async {
    if (podPlayerController.value.isInitialised) {
      podPlayerController.value.pause();
    } else if (mkVideoController != null) {
      mkVideoController!.player.pause();
  ...
  Future<void> onClose() async {
    ...
    if (mkVideoController != null) {
      mkVideoController!.player.dispose();
    }
    adPlayer.dispose();

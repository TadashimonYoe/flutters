...
// Global fit notifier allowing toggle between cover/contain in fullscreen.
final ValueNotifier<BoxFit> yGlobalFitNotifier = ValueNotifier<BoxFit>(BoxFit.cover);
...
class YMaterialVideoControlsThemeData {
  ...
  const YMaterialVideoControlsThemeData({
    ...
    this.bottomButtonBarMargin = const EdgeInsets.only(left: 16.0, right: 8.0, bottom: 12.0),
...
class _YMaterialFullscreenButtonState extends State<YMaterialFullscreenButton> {
  ...
  Future<void> toggleScreenMode(BuildContext context) async {
    if (!isFullScreen) {
      ...
      yGlobalFitNotifier.value = BoxFit.cover;
                    ...
                    child: FullscreenInheritedWidget(
                      ...
                      child: VideoStateInheritedWidget(
                        ...
                        child: ValueListenableBuilder<BoxFit>(
                          valueListenable: yGlobalFitNotifier,
                          builder: (ctx2, fit, _) => Video(
                            controller: controllerValue,
                            fit: fit,
                            alignment: Alignment.center,
                            aspectRatio: MediaQuery.of(ctx).size.aspectRatio,
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      icon: widget.icon ?? (isFullscreen(context) ? const Icon(Icons.fullscreen_exit_rounded) : const Icon(Icons.fullscreen_rounded)),
      iconSize: widget.iconSize ?? _theme(context).buttonBarButtonSize,
      color: widget.iconColor ?? _theme(context).buttonBarButtonColor,
    );
  }
...
class YMaterialBackButton extends StatelessWidget {
  const YMaterialBackButton({super.key});
...
class YMaterialBestFitButton extends StatefulWidget {
  const YMaterialBestFitButton({super.key});

  @override
  State<YMaterialBestFitButton> createState() => _YMaterialBestFitButtonState();
}

class _YMaterialBestFitButtonState extends State<YMaterialBestFitButton> {
  BoxFit current = yGlobalFitNotifier.value;

  void toggleFit() {
    current = current == BoxFit.cover ? BoxFit.contain : BoxFit.cover;
    yGlobalFitNotifier.value = current;
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      tooltip: current == BoxFit.cover ? 'Mejor ajuste' : 'Llenar pantalla',
      iconSize: _theme(context).buttonBarButtonSize,
      color: _theme(context).buttonBarButtonColor,
    );
  }

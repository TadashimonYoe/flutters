...
class CoreServiceApis {
  ...
  Future<PaymentSettingResponse> getPaymentSettings() async {
    final dynamic res = await handleResponse(
      await buildHttpResponse(
        getEndPoint(endPoint: APIEndPoints.paymentSettings),
        manageApiVersion: true,
      ),
    );

    if (res is Map<String, dynamic>) {
      return PaymentSettingResponse.fromJson(res);
    } else if (res is List) {
      return PaymentSettingResponse(status: true, message: "", data: List<PaymentSetting>.from(res.map((x) => PaymentSetting.fromJson(x))));
    } else {
      return PaymentSettingResponse(status: false, message: "", data: []);
    }
  }
...
// Add/Edit Rating
  ...
  Future<SubscriptionResponseModel> saveSubscriptionDetails({required Map<String, dynamic> request, File? proofFile}) async {
    if (proofFile != null) {
      final multiPartRequest = await getMultiPartRequest(APIEndPoints.saveSubscriptionDetails);
      multiPartRequest.fields.addAll(await getMultipartFields(val: request));
      multiPartRequest.files.add(
        await http.MultipartFile.fromPath('proof_file', proofFile.path.validate()),
      );
      ...
      await sendMultiPartRequest(
        ...
        onSuccess: (temp) async {
          respJson = jsonDecode(temp);
        },
        onError: (error, response) {
          throw error;
        },
      );
      return SubscriptionResponseModel.fromJson(respJson);
    } else {
...
//Page List Setting Screen
  ...
  Future<Map<String, dynamic>> submitTadashiPayment({required Map<String, dynamic> request, List<File>? files}) async {
    final multiPartRequest = await getMultiPartRequest(
      APIEndPoints.tadashiSubmitPayment,
      baseUrl: buildBaseUrl(APIEndPoints.tadashiSubmitPayment, manageApiVersion: false).toString(),
    );
    multiPartRequest.fields.addAll(await getMultipartFields(val: request));
    if (files.validate().isNotEmpty) {
      multiPartRequest.files.add(
